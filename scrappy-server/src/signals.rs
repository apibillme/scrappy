...
pub(crate) struct Signals {
    ...
    #[cfg(unix)]
    streams: Vec<(Signal, scrappy_rt::signal::unix::Signal)>,
}

impl Signals {
    pub(crate) fn start(srv: Server) -> io::Result<()> {
        scrappy_rt::spawn(lazy(|_| {
            #[cfg(not(unix))]
            {
                scrappy_rt::spawn(Signals {
                    srv,
                    stream: Box::pin(scrappy_rt::signal::ctrl_c()),
                });
            }
            #[cfg(unix)]
            {
                use scrappy_rt::signal::unix;
                ...
                scrappy_rt::spawn(Signals { srv, streams })
            }
        }));
