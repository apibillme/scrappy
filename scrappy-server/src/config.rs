...
use scrappy_service as scrappy;
use scrappy_utils::counter::CounterGuard;
        ...
        async move {
            ...
            for token in tokens {
                if let Some(srv) = services.remove(&token) {
                    ...
                    res.push((
                        token,
                        Box::new(StreamService::new(scrappy::fn_service(
                            move |_: TcpStream| {
...
impl ServiceRuntime {
    ...
    where
        F: scrappy::IntoServiceFactory<T>,
        T: scrappy::ServiceFactory<Config = (), Request = TcpStream> + 'static,
        T::Future: 'static,
        T::Service: 'static,
...
impl<T> scrappy::ServiceFactory for ServiceFactory<T>
where
    T: scrappy::ServiceFactory<Config = (), Request = TcpStream>,
    T::Future: 'static,
    T::Service: 'static,
    T::Error: 'static,
