...
use scrappy_http::{body, http::StatusCode, ws, Error, HttpService, Response};
use scrappy_http_test::test_server;
use scrappy_service::{pipeline_factory, IntoServiceFactory, ServiceFactory};
use scrappy_utils::framed::Dispatcher;
use bytes::Bytes;
use futures::{future, SinkExt, StreamExt};

use scrappy_framed::{FramedApp, FramedRequest, FramedRoute, SendError, VerifyWebSockets};
...
async fn test_service() {
    let mut srv = test_server(|| {
        pipeline_factory(scrappy_http::h1::OneRequest::new().map_err(|_| ())).and_then(
            pipeline_factory(
