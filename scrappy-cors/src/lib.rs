...
use scrappy::dev::{RequestHead, ServiceRequest, ServiceResponse};
use scrappy::error::{Error, ResponseError, Result};
use scrappy::http::header::{self, HeaderName, HeaderValue};
use scrappy::http::{self, Error as HttpError, Method, StatusCode, Uri};
...
impl Cors {
    ...
    /// CredentialsWithWildcardOrigin` error during scrappy launch or runtime.
    ///
    /// Defaults to `false`.
...
#[cfg(test)]
mod tests {
    use scrappy_service::{fn_service, Transform};
