...
use scrappy_codec::{AsyncRead, AsyncWrite, Framed};
use scrappy_http::http::header::{ContentType, Header, HeaderName, IntoHeaderValue};
use scrappy_http::http::{Error as HttpError, Method, StatusCode, Uri, Version};
use scrappy_http::test::TestRequest as HttpTestRequest;
use scrappy_http::{cookie::Cookie, ws, Extensions, HttpService, Request};
use scrappy_router::{Path, ResourceDef, Url};
use scrappy_rt::{time::delay_for, System};
use scrappy_service::{
    map_config, IntoService, IntoServiceFactory, Service, ServiceFactory,
};
use scrappy_client::error::PayloadError;
use scrappy_client::{Client, ClientRequest, ClientResponse, Connector};
...
pub use scrappy_http::test::TestBuffer;
...
///
/// ```rust
/// use scrappy_service::Service;
/// use scrappy::{test, web, App, HttpResponse, http::StatusCode};
///
/// #[scrappy_rt::test]
...
///
/// ```rust
/// use scrappy::{test, App, HttpResponse, http::StatusCode};
/// use scrappy_service::Service;
///
/// #[test]
/// fn test_response() {
...
///
/// ```rust
/// use scrappy::{test, web, App, HttpResponse, http::header};
/// use bytes::Bytes;
///
/// #[scrappy_rt::test]
/// async fn test_index() {
...
///
/// ```rust
/// use scrappy::{App, test, web, HttpResponse, http::header};
...
///
/// For unit testing, scrappy provides a request builder type and a simple handler runner. TestRequest implements a builder-like pattern.
/// You can generate various types of request via TestRequest's methods:
///  * `TestRequest::to_request` creates `scrappy_http::Request` instance.
...
///
/// ```rust
/// use scrappy::{test, HttpRequest, HttpResponse, HttpMessage};
/// use scrappy::http::{header, StatusCode};
///
...
/// integration tests cases for scrappy web applications.
///
/// # Examples
///
/// ```rust
/// use scrappy::{web, test, App, HttpResponse, Error};
///
    ...
    B: MessageBody + 'static,
{
    ...
    thread::spawn(move || {
        let sys = System::new("scrappy-test-server");
...
pub struct TestServer {
    addr: net::SocketAddr,
    client: scrappy_client::Client,
    system: scrappy_rt::System,
    ssl: bool,
    server: Server,
}
...
impl TestServer {
    ...
    pub async fn ws_at(
        &mut self,
        path: &str,
    ) -> Result<Framed<impl AsyncRead + AsyncWrite, ws::Codec>, scrappy_client::error::WsClientError>
    {
...
#[cfg(test)]
mod tests {
    use scrappy_http::httpmessage::HttpMessage;
    ...
    #[scrappy_rt::test]
    async fn test_actor() {
        use scrappy_actor::Actor;

        struct MyActor;

        struct Num(usize);
        impl scrappy_actor::Message for Num {
            type Result = usize;
        }
        impl scrappy_actor::Actor for MyActor {
            type Context = scrappy_actor::Context<Self>;
        }
        impl scrappy_actor::Handler<Num> for MyActor {
            type Result = usize;
