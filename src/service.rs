...
use scrappy_http::body::{Body, MessageBody, ResponseBody};
use scrappy_http::http::{HeaderMap, Method, StatusCode, Uri, Version};
use scrappy_http::{
    Error, Extensions, HttpMessage, Payload, PayloadStream, RequestHead, Response,
    ResponseHead,
};
use scrappy_router::{IntoPattern, Path, Resource, ResourceDef, Url};
use scrappy_service::{IntoServiceFactory, ServiceFactory};
...
impl ServiceRequest {
    ...
    /// scrappy http server, then peer address would be address of this proxy.
    ///
...
impl WebService {
    ...
    ///
    /// ```rust
    /// use scrappy::{web, guard, dev, App, Error, HttpResponse};
    ///
