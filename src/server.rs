...
use scrappy_http::{body::MessageBody, Error, HttpService, KeepAlive, Request, Response};
use scrappy_server::{Server, ServerBuilder};
use scrappy_service::{map_config, IntoServiceFactory, Service, ServiceFactory};

use net2::TcpBuilder;

#[cfg(unix)]
use scrappy_http::Protocol;
#[cfg(unix)]
use scrappy_service::pipeline_factory;
#[cfg(unix)]
use futures::future::ok;

#[cfg(feature = "openssl")]
use scrappy_tls::openssl::{AlpnError, SslAcceptor, SslAcceptorBuilder};
#[cfg(feature = "rustls")]
use scrappy_tls::rustls::ServerConfig as RustlsServerConfig;
...
///
/// ```rust,no_run
/// use scrappy::{web, App, HttpResponse, HttpServer};
///
/// #[scrappy_rt::main]
    ...
    B: MessageBody + 'static,
{
    ...
    pub fn maxconnrate(self, num: usize) -> Self {
        scrappy_tls::max_concurrent_ssl_connect(num);
        self
    }
    ...
    pub fn listen(mut self, lst: net::TcpListener) -> io::Result<Self> {
        ...
        self.builder = self.builder.listen(
            format!("scrappy-web-service-{}", addr),
            lst,
            move || {
    ...
    pub fn listen_uds(
        ...
        let addr = format!("scrappy-web-service-{:?}", lst.local_addr()?);
        ...
        A: AsRef<std::path::Path>,
    {
        ...
        self.builder = self.builder.bind_uds(
            format!("scrappy-web-service-{:?}", addr.as_ref()),
            addr,
            move || {
                let c = cfg.lock().unwrap();
                let config = AppConfig::new(
    ...
    B: MessageBody,
{
    ...
    /// `accept()` in a loop.
    ///
    /// This methods panics if no socket address can be bound or an `scrappy` system is not yet
    /// configured.
    ///
    /// ```rust,no_run
    /// use std::io;
