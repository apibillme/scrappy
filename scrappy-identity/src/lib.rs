...
use scrappy_service::{Service, Transform};
use futures::future::{ok, FutureExt, LocalBoxFuture, Ready};
use serde::{Deserialize, Serialize};
use time::Duration;

use scrappy::cookie::{Cookie, CookieJar, Key, SameSite};
use scrappy::dev::{Extensions, Payload, ServiceRequest, ServiceResponse};
use scrappy::error::{Error, Result};
use scrappy::http::header::{self, HeaderValue};
use scrappy::{FromRequest, HttpMessage, HttpRequest};

/// The extractor type to obtain your identity from a request.
///
/// ```rust
/// use scrappy::*;
/// use scrappy_identity::Identity;
///
...
/// ```rust
/// use scrappy::App;
/// use scrappy_identity::{CookieIdentityPolicy, IdentityService};
///
/// fn main() {
///     let app = App::new().wrap(IdentityService::new(
///         // <- create identity middleware
        ...
        async move {
            match fut.await {
                Ok(id) => {
                    ...
                    // https://github.com/scrappy/scrappy-web/issues/1263
...
impl CookieIdentityInner {
    fn new(key: &[u8]) -> CookieIdentityInner {
        ...
        CookieIdentityInner {
            ...
            name: "scrappy-identity".to_owned(),
            path: "/".to_owned(),
            domain: None,
            secure: true,
            max_age: None,
...
mod tests {
    ...
    use super::*;
    use scrappy_service::into_service;
    use scrappy::http::StatusCode;
    use scrappy::test::{self, TestRequest};
    use scrappy::{error, web, App, Error, HttpResponse};

    const COOKIE_KEY_MASTER: [u8; 32] = [0; 32];
    const COOKIE_NAME: &'static str = "scrappy_auth";
    ...
    >(
        f: F,
    ) -> impl scrappy_service::Service<
        Request = scrappy_http::Request,
        Response = ServiceResponse<scrappy::body::Body>,
        Error = Error,
    > {
        test::init_service(
            App::new()
    ...
    async fn test_borrowed_mut_error() {
        ...
        let mut srv = IdentityServiceMiddleware {
            ...
            service: Rc::new(RefCell::new(into_service(|_: ServiceRequest| {
                async move {
                    scrappy_rt::time::delay_for(std::time::Duration::from_secs(100)).await;
        ...
        scrappy_rt::spawn(async move {
            let _ = srv2.call(req).await;
        });
        scrappy_rt::time::delay_for(std::time::Duration::from_millis(50)).await;
