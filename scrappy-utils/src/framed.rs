...
use scrappy_codec::{AsyncRead, AsyncWrite, Decoder, Encoder, Framed};
use scrappy_service::{IntoService, Service};
        ...
        loop {
            match self.service.poll_ready(cx) {
                Poll::Ready(Ok(_)) => {
                    ...
                    scrappy_rt::spawn(self.service.call(item).map(move |item| {
