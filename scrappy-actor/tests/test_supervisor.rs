...
impl scrappy_actor::Supervised for MyActor {
    fn restarting(&mut self, _: &mut scrappy_actor::Context<MyActor>) {
        self.1.fetch_add(1, Ordering::Relaxed);
    }
}

impl scrappy_actor::Handler<Die> for MyActor {
    type Result = ();

    fn handle(&mut self, _: Die, ctx: &mut scrappy_actor::Context<MyActor>) {
...
fn test_supervisor_restart() {
    ...
    System::run(move || {
        let addr =
            scrappy_actor::Supervisor::start(move |_| MyActor(starts2, restarts2, messages2));
        addr.do_send(Die);
        addr.do_send(Die);
        *addr2.lock().unwrap() = Some(addr);

        scrappy_actor::spawn(async move {
