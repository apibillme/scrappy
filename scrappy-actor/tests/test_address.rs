...
impl scrappy_actor::Handler<Ping> for MyActor {
    type Result = ();
...
impl scrappy_actor::Handler<Ping> for MyActor3 {
    type Result = ();

    fn handle(&mut self, _: Ping, _: &mut scrappy_actor::Context<MyActor3>) -> Self::Result {
        System::current().stop();
    }
}
