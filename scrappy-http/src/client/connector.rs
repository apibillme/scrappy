...
use scrappy_codec::{AsyncRead, AsyncWrite};
use scrappy_connect::{
    default_connector, Connect as TcpConnect, Connection as TcpConnection,
};
use scrappy_rt::net::TcpStream;
use scrappy_service::{apply_fn, Service};
use scrappy_utils::timeout::{TimeoutError, TimeoutService};
use http::Uri;
...
#[cfg(feature = "openssl")]
use scrappy_connect::ssl::openssl::SslConnector as OpensslConnector;

#[cfg(feature = "rustls")]
use scrappy_connect::ssl::rustls::ClientConfig;
#[cfg(feature = "rustls")]
use std::sync::Arc;
...
impl Connector<(), ()> {
    ...
    pub fn new() -> Connector<
        impl Service<
                ...
                Error = scrappy_connect::ConnectError,
            > + Clone,
        TcpStream,
    > {
        let ssl = {
            #[cfg(feature = "openssl")]
            {
                use scrappy_connect::ssl::openssl::SslMethod;
            ...
            #[cfg(all(not(feature = "openssl"), feature = "rustls"))]
            {
                ...
                config
                    .root_store
                    .add_server_trust_anchors(&scrappy_tls::rustls::TLS_SERVER_ROOTS);
        ...
        + 'static,
{
    ...
    pub fn finish(
        ...
        #[cfg(any(feature = "openssl", feature = "rustls"))]
        {
            const H2: &[u8] = b"h2";
            #[cfg(feature = "openssl")]
            use scrappy_connect::ssl::openssl::OpensslConnector;
            #[cfg(feature = "rustls")]
            use scrappy_connect::ssl::rustls::{RustlsConnector, Session};
            use scrappy_service::{boxed::service, pipeline};
