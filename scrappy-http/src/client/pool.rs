...
use scrappy_rt::time::{delay_for, Delay};
use scrappy_service::Service;
use scrappy_utils::{oneshot, task::LocalWaker};
use bytes::Bytes;
        ...
        + 'static,
{
    ...
    fn call(&mut self, req: Connect) -> Self::Future {
        ...
        scrappy_rt::spawn(ConnectorPoolSupport {
            ...
            match poll_fn(|cx| Poll::Ready(inner.borrow_mut().acquire(&key, cx))).await {
                ...
                Acquire::Available => {
                    ...
                    if proto == Protocol::Http1 {
                        ...
                        scrappy_rt::spawn(connection.map(|_| ()));
                        Ok(IoConnection::new(
                    ...
                    || (now - conn.created) > self.conn_lifetime
                {
                    if let Some(timeout) = self.disconnect_timeout {
                        if let ConnectionType::H1(io) = conn.io {
                            scrappy_rt::spawn(CloseConnection::new(io, timeout))
                        }
                    }
                } else {
                    let mut io = conn.io;
                            ...
                            Poll::Ready(Ok(n)) if n > 0 => {
                                if let Some(timeout) = self.disconnect_timeout {
                                    if let ConnectionType::H1(io) = io {
                                        scrappy_rt::spawn(CloseConnection::new(
                                            io, timeout,
                                        ))
                                    }
                                }
    ...
    Io: AsyncRead + AsyncWrite + Unpin + 'static,
{
    fn spawn(
        ...
        scrappy_rt::spawn(OpenWaitingConnection {
            key,
            fut,
            h2: None,
            rx: Some(rx),
