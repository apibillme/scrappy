...
use scrappy_http::body::Body;
use scrappy_http::cookie::{Cookie, CookieJar, USERINFO};
use scrappy_http::http::header::{self, Header, IntoHeaderValue};
use scrappy_http::http::{
    uri, ConnectionType, Error as HttpError, HeaderMap, HeaderName, HeaderValue, Method,
    Uri, Version,
};
use scrappy_http::{Error, RequestHead};
...
/// #[scrappy_rt::main]
/// async fn main() {
///    let response = scrappy_client::Client::new()
...
impl ClientRequest {
    ...
    /// ```rust
    /// fn main() {
    /// # scrappy_rt::System::new("test").block_on(futures::future::lazy(|_| {
    ///     let req = scrappy_client::Client::new()
    ///         .get("http://www.rust-lang.org")
    ///         .set(scrappy_client::http::header::Date::now())
    ///         .set(scrappy_client::http::header::ContentType(mime::TEXT_HTML));
    /// #   Ok::<_, ()>(())
    /// # }));
    /// }
    /// ```
    ...
    ///
    /// ```rust
    /// use scrappy_client::{http, Client};
    ///
    /// fn main() {
    /// # scrappy_rt::System::new("test").block_on(async {
    ///     let req = Client::new()
    ///         .get("http://www.rust-lang.org")
    ///         .header("X-TEST", "value")
    ...
    /// #[scrappy_rt::main]
    /// async fn main() {
    ///     let resp = scrappy_client::Client::new().get("https://www.rust-lang.org")
    ///         .cookie(
    ///             scrappy_client::http::Cookie::build("name", "value")
...
mod tests {
    ...
    fn test_basics() {
        let mut req = Client::new()
            ...
            .if_true(true, |req| req.header(header::SERVER, "scrappy_client"))
            .if_true(false, |req| req.header(header::EXPECT, "scrappy_client"))
