...
use scrappy_service::{map_config, pipeline_factory, ServiceFactory};
use scrappy::http::Version;
use scrappy::{dev::AppConfig, web, App, HttpResponse};
...
async fn _test_connection_reuse_h2() {
    ...
    let client = scrappy_client::Client::build()
        .connector(scrappy_client::Connector::new().rustls(Arc::new(config)).finish())
        .finish();
