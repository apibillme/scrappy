...
use scrappy_http::HttpService;
use scrappy_http_test::test_server;
use scrappy_service::{map_config, pipeline_factory};
use scrappy::dev::{AppConfig, BodyEncoding};
use scrappy::http::Cookie;
use scrappy::middleware::Compress;
use scrappy::{
    http::header, test, web, App, Error, HttpMessage, HttpRequest, HttpResponse,
};
use scrappy_client::error::SendRequestError;
...
async fn test_timeout() {
    let srv = test::start(|| {
        App::new().service(web::resource("/").route(web::to(|| {
            async {
                scrappy_rt::time::delay_for(Duration::from_millis(200)).await;
                Ok::<_, Error>(HttpResponse::Ok().body(STR))
            }
        })))
    });

    let connector = scrappy_client::Connector::new()
        .connector(scrappy_connect::new_connector(
            scrappy_connect::start_default_resolver(),
        ))
        .timeout(Duration::from_secs(15))
        .finish();

    let client = scrappy_client::Client::build()
        .connector(connector)
...
async fn test_connection_reuse() {
    ...
    let client = scrappy_client::Client::default();
...
async fn test_connection_wait_queue() {
    ...
    let client = scrappy_client::Client::build()
        .connector(scrappy_client::Connector::new().limit(1).finish())
        .finish();
...
async fn test_with_query_parameter() {
    ...
    let res = scrappy_client::Client::new()
        .get(srv.url("/?qp=5"))
        .send()
        .await
        .unwrap();
...
async fn test_no_decompress() {
    ...
    let mut res = scrappy_client::Client::new()
        .get(srv.url("/"))
        .no_decompress()
        .send()
        .await
...
//     let mut sys = scrappy::System::new("test");
