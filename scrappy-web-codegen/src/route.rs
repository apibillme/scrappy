...
impl Route {
    ...
    pub fn generate(&self) -> TokenStream {
        ...
        let stream = quote! {
            ...
            impl scrappy::dev::HttpServiceFactory for #name {
                fn register(self, config: &mut scrappy::dev::AppService) {
                    #ast
                    let resource = scrappy::Resource::new(#path)
                        .name(#resource_name)
                        .guard(scrappy::guard::#guard())
                        #(.guard(scrappy::guard::fn_guard(#extra_guards)))*
                        .#resource_type(#name);

                    scrappy::dev::HttpServiceFactory::register(resource, config)
                }
            }
        };
        stream.into()
