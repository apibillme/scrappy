use scrappy::{http, test, web::Path, App, HttpResponse, Responder};
use scrappy_web_codegen::{connect, delete, get, head, options, patch, post, put, trace};
...
#[get("/test")]
fn auto_async() -> impl Future<Output = Result<HttpResponse, scrappy::Error>> {
    future::ok(HttpResponse::Ok().finish())
}

#[get("/test")]
fn auto_sync() -> impl Future<Output = Result<HttpResponse, scrappy::Error>> {
    future::ok(HttpResponse::Ok().finish())
}
