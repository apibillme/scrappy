...
use scrappy_service::boxed::{self, BoxService, BoxServiceFactory};
use scrappy_service::{IntoServiceFactory, Service, ServiceFactory};
use scrappy::dev::{
    AppService, HttpServiceFactory, Payload, ResourceDef, ServiceRequest,
    ServiceResponse,
};
use scrappy::error::{BlockingError, Error, ErrorInternalServerError};
use scrappy::guard::Guard;
use scrappy::http::header::{self, DispositionType};
use scrappy::http::Method;
use scrappy::{web, FromRequest, HttpRequest, HttpResponse};
use bytes::Bytes;
...
impl Files {
    ...
    /// Pool size can be changed by setting scrappy_THREADPOOL environment variable.
    ...
    fn call(&mut self, req: ServiceRequest) -> Self::Future {
        ...
        if !is_method_valid {
            return Either::Left(ok(req.into_response(
                scrappy::HttpResponse::MethodNotAllowed()
...
mod tests {
    ...
    async fn test_named_file_content_length_headers() {
        // use scrappy::body::{MessageBody, ResponseBody};
