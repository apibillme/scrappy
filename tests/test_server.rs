...
use scrappy::dev::BodyEncoding;
use scrappy::middleware::Compress;
use scrappy::{dev, test, web, App, Error, HttpResponse};
...
impl TestBody {
    fn new(data: Bytes, chunk_size: usize) -> Self {
        TestBody {
            data,
            chunk_size,
            delay: scrappy_rt::time::delay_for(std::time::Duration::from_millis(10)),
        }
    }
}
...
impl futures::Stream for TestBody {
    ...
    fn poll_next(
        ...
        self.delay = scrappy_rt::time::delay_for(std::time::Duration::from_millis(10));
...
async fn test_body_encoding_override() {
    let srv = test::start_with(test::config().h1(), || {
        App::new()
            ...
            .service(web::resource("/raw").route(web::to(|| {
                let body = scrappy::dev::Body::Bytes(STR.into());
                let mut response =
                    HttpResponse::with_body(scrappy::http::StatusCode::OK, body);
    ...
    // Raw Response
    let mut response = srv
        .request(scrappy::http::Method::GET, srv.url("/raw"))
        .no_decompress()
        .header(ACCEPT_ENCODING, "deflate")
        .send()
        .await
...
async fn test_brotli_encoding_large_openssl() {
    ...
    let srv = test::start_with(test::config().openssl(builder.build()), move || {
        App::new().service(web::resource("/").route(web::to(|bytes: Bytes| {
            HttpResponse::Ok()
                .encoding(scrappy::http::ContentEncoding::Identity)
                .body(bytes)
        })))
    });
    ...
    let mut response = srv
        .post("/")
        .header(scrappy::http::header::CONTENT_ENCODING, "br")
        .send_body(enc)
        .await
        .unwrap();
    assert!(response.status().is_success());
...
async fn test_reading_deflate_encoding_large_random_rustls() {
    ...
    let req = srv
        .post("/")
        .header(scrappy::http::header::CONTENT_ENCODING, "deflate")
